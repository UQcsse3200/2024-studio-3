...
public class ForestGameArea extends GameArea {
    ...
    private void spawnGhosts() {
        ...
        // Define different target positions for each ghost
        Vector2 targetPos1 = new Vector2(5, 5); // First ghost target position
        Vector2 targetPos2 = new Vector2(7, 8); // Second ghost target position

        for (int i = 0; i < NUM_GHOSTS; i++) {
            GridPoint2 randomPos = RandomUtils.random(minPos, maxPos);

            // Assign different target positions based on the ghost index
            Vector2 targetPosition = (i == 0) ? targetPos1 : targetPos2;

            Entity ghost = NPCFactory.createGhost(player, targetPosition); // Pass target position
  ...
  private void spawnGhostKing() {
    ...
    GridPoint2 maxPos = terrain.getMapBounds(0).sub(2, 2);
        ...
        Entity ghostKing = NPCFactory.createGhostKing(player, targetPos3); // Pass target position
